...
void SC_CurrentStore::aPrepareSql()
{
    ...
    m_sqlQuery = QString("select g.id, gg.fname as fgroup, g.fname as fgoods, coalesce(i.qty, 0)-coalesce(o.qty, 0) as qty, \
            ...
            left join (select bb.fgoods, sum(fqty) as qty from stb bb left join sth hh on hh.id=bb.fdoc where hh.fdate<=%1 and hh.ftype=1 group by 1) o on o.fgoods=g.id \
            left join (select bb.fgoods, sum(fqty) as qty from stb bb left join sth hh on hh.id=bb.fdoc where hh.fdate<=%1 and hh.ftype=2 group by 1) i on i.fgoods=g.id \
